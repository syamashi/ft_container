...
struct _Rb_tree_header {
  ...
  void _M_move_data(_Rb_tree_header& __from) {
    ...
    _M_header._M_parent->_M_parent = &_M_header;  // 自分が親
...
struct _Rb_tree_node : public _Rb_tree_node_base {
  ...
  _Val* _M_valptr() {
    return __builtin_addressof(_M_value_field);
  }  // == std::__addressof(_M_value_field)

  const _Val* _M_valptr() const { return __builtin_addressof(_M_value_field); }
};
...
struct _Rb_tree_iterator {
  ...
  _Rb_tree_node_base* local_Rb_tree_increment(_Rb_tree_node_base* __x) throw() {
    if (__x->_M_right != 0) {
      __x = __x->_M_right;
      while (__x->_M_left != 0) __x = __x->_M_left;
    } else {
  ...
  _Rb_tree_node_base* local_Rb_tree_decrement(_Rb_tree_node_base* __x) throw() {
